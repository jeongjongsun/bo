-- 주문 샘플 10만 건 (마스터 10만 행 + 라인 10만 행, 주문당 1라인)
...
-- 중복 실행 시 PK/UK 위반. 재삽입 시 기존 해당 구간 삭제 후 실행
-- 실행 시간: 환경에 따라 수 분 소요될 수 있음

delete from om_order_m;
delete from om_order_item_m;

-- 1) om_order_m (주문 마스터 10만 건)
...
SELECT
    ...
    'ORD-2025-' || lpad(g.id::text, 6, '0') AS order_no,
    CASE WHEN g.id % 5 = 0 THEN 'SHP-' || lpad((g.id / 5)::text, 5, '0') ELSE NULL END AS combined_ship_no,
    ...
    '010-' || lpad((1111 + (g.id % 9000))::text, 4, '0') || '-' || lpad((g.id * 37 % 10000)::text, 4, '0') AS receiver_tel,
    '서울시 강남구 테헤란로 ' || (100 + (g.id % 9900)) || ' (샘플주소)' AS receiver_addr,
    ((g.id - 1) % 20) || '동 ' || (1 + (g.id - 1) % 30) || '호' AS receiver_addr2,
...
-- 2) om_order_item_m (주문당 1라인, 10만 행)
...
SELECT
    ...
    ('2025-01-01'::date + ((g.id - 1) * 7 % 90) * interval '1 day')::date AS regist_dt,
    1 AS line_no,
    'ITEM-2025-' || lpad(g.id::text, 6, '0') AS item_order_no,
...
SELECT setval('seq_order_id', 100000);
